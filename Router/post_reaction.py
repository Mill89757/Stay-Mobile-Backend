...
async def create_post_reaction(post_reaction:schemas.PostReactionCreate, db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
    print(current_user)
...
async def get_all_reactions(skip:int = 0, limit:int = 100, db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
...
async def get_reactions_by_post_id(post_id:int, db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
...
async def get_reactions_by_emoji(emoji_image:str, db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
...
async def get_reaction_by_postid_emoji(post_id:int, emoji_image:str, db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
...
async def update_post_reaction(post_id:int, emoji_image:str, db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
...
async def delet_reaction(post_id:int, emoji_image:str, db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
    print(current_user)
...
async def update_count(post_id:int, emoji_image:str, action:bool, db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
...
async def get_reaction_count_challenge_id(challenge_id:int, db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
