...
def top3categories(user_id: int) -> set:
    ...
    contribution = byte_to_utf8(redis_client.hget(
        'user_contribution', user_id), StrToSplit=',', ifError=['0']*5)
...
def filtered_posts_from_reacted_challenges(user_id: int) -> list:
    ...
    for post_id in recent_posts:
        ...
        category = int(byte_to_utf8(redis_client.hget(
            'on_clg_info', clg_id), StrToSplit=',')[0])
        if category not in top3:
            interacted_post = redis_client.sismember(
                f'{user_id}_liked_posts', post_id)
            interacted_clg = redis_client.zscore(
...
def filtered_posts_from_top3_categories(user_id: int) -> list:
    ...
    for category_code in top3:
        ...
        raw_post_pool = redis_client.zrangebyscore(
        ...
        for post_id in raw_post_pool:
            if not redis_client.sismember(f'{user_id}_liked_posts', post_id):
...
def get_recommended_post(user_id):
    ...
    return filtered_posts_from_reacted_challenges(user_id) + filtered_posts_from_top3_categories(user_id)
