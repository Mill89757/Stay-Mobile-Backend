...
def get_post_content(db: Session, post_content_id: int):
    ...
    post_content = db.query(models.PostContent).filter(
        models.PostContent.id == post_content_id).first()
    if post_content is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="post_content not found")
    return post_content
...
def update_post_content(db: Session, post_content_id: int, post_content: schemas.PostContentCreate):
    ...
    db_post_content = db.query(models.PostContent).filter(
        models.PostContent.id == post_content_id).first()
