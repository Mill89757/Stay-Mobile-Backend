...
def create_tracking(db: Session, tracking: schemas.TrackingsRequest):
    ...
    db_challenge = db.query(models.Challenge).filter(
        models.Challenge.id == tracking.challenge_id).first()
    if db_challenge.challenge_owner_id != tracking.owner_id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="This user is not the owner of this challenge")
...
def read_tracking_by_challenge_id(db: Session, challenge_id: int):
    ...
    result = db.query(models.Tracking).filter(
        models.Tracking.challenge_id == challenge_id).all()
    return result
...
def read_follower_by_challenge_id(db: Session, challenge_id: int):
    ...
    challenge_tracking = db.query(models.Challenge, models.Tracking)\
        ...
        .order_by(models.Tracking.created_time)\
        .filter(models.Tracking.challenge_id == challenge_id).limit(10).all()
    follower_ids = [record[1].follower_id for record in challenge_tracking]
    result = []
    for follower_id in follower_ids:
        follower_user = db.query(models.User).filter(
            models.User.id == follower_id).first()
...
def read_activated_tracking_challenge_data_by_follower_id(db: Session, follower_id: int):
    ...
    db_activated_tracking = db.query(models.Tracking).filter(
        models.Tracking.follower_id == follower_id).filter(not models.Tracking.is_terminated).all()
...
def update_tracking_status(
        db: Session, challenge_id: int, follower_id: int, tracking: schemas.TrackingsRequest):
    ...
    db_tracking = db.query(models.Tracking).filter(models.Tracking.challenge_id == challenge_id)\
        .filter(models.Tracking.follower_id == follower_id).first()
    if db_tracking is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Tracking not found")
    db.query(models.Tracking).filter(models.Tracking.challenge_id == challenge_id)\
        .filter(models.Tracking.follower_id == follower_id).update(
        {
...
def delete_tracking(db: Session, tracking_id: int):
    ...
    db_tracking = db.query(models.Tracking).filter(
        models.Tracking.id == tracking_id).first()
    if db_tracking is None:
        return None
