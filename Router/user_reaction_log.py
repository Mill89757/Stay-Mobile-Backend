...
async def create_user_reaction_log(log:schemas.UserReactionLogCreate, db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
...
async def get_all_log(db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
    print(current_user)
...
async def get_log_by_log_id(log_id:int, db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
    print(current_user)
...
async def get_log_by_user_id(user_id:int, db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
    print(current_user)
...
async def get_log_by_post_id(post_id:int, db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
...
async def get_log_by_emoji_image(emoji_image:str, db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
...
async def get_latest_log_of_user(post_id:int, user_id:int, db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
...
async def update_reaction_log(log_id:int, log:schemas.UserReactionLogRead, db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
...
async def delete_log(log_id:int,  db:Session=Depends(get_db), current_user: dict = Depends(verify_token)):
    print(current_user)
