...
async def get_course_data(db: Session = Depends(get_db),current_user: dict = Depends(verify_token)):
...
async def get_course_by_id(id: int, db: Session = Depends(get_db),current_user: dict = Depends(verify_token)):
    """Read course by id
...
async def update_challenge_course_id(course_id:int, challenge_id:int, db: Session=Depends(get_db),current_user: dict = Depends(verify_token)):
