...
async def create_post_content_router(post_content:schemas.PostContentCreate, db: Session = Depends(get_db), current_user: dict = conditional_depends(depends=verify_token)):
...
async def get_post_content_route(post_content_id: int, db: Session = Depends(get_db),current_user: dict = conditional_depends(depends=verify_token)):
...
async def get_post_content_route_by_post_id(post_id: int, db: Session = Depends(get_db),current_user: dict = conditional_depends(depends=verify_token)):
...
async def get_all_post_content_route(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user: dict = conditional_depends(depends=verify_token)):
    print(current_user)
...
async def update_post_content_route(post_content_id: int, post_content: schemas.PostContentCreate, db: Session = Depends(get_db), current_user: dict = conditional_depends(depends=verify_token)):
...
async def delete_post_route(post_content_id: int, db: Session = Depends(get_db),current_user: dict = conditional_depends(depends=verify_token)):
